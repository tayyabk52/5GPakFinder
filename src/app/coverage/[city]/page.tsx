...
export default async function CityCoveragePage({ params }: { params: Promise<{ city: string }> }) {
  ...
  return <CoveragePageFrame>
    ...
    <EvidenceNote>Source records were retrieved {formatCoverageDate(facts.retrievedAt)} and reviewed {formatCoverageDate(facts.reviewedAt)}. Counts are not a network quality ranking and can differ from an operator&apos;s current live network. Dataset use is governed by <a href={DATASET_LICENSE_PATH} className="font-semibold text-[#126c85] underline underline-offset-2">{DATASET_LICENSE_NAME}</a>.</EvidenceNote>
