...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const navigation = (mobile = false) => <nav className={mobile ? "space-y-2" : collapsed ? "space-y-1" : "space-y-1.5"} aria-label="Primary navigation">
    ...
    {items.map(({ href, label, icon: Icon }) => {
      ...
      return <Link key={href} href={href} onClick={mobile ? () => setOpen(false) : undefined} aria-current={selected ? "page" : undefined} title={compact ? label : undefined} className={`group relative flex items-center rounded-xl transition-[background-color,color,transform] duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#49cbeb] ${compact ? "mx-auto h-9 w-9 justify-center rounded-lg px-0" : mobile ? "min-h-[3.25rem] gap-3 rounded-2xl px-3.5" : "min-h-11 gap-3 px-3"} ${selected ? "bg-slate-950 text-white shadow-[0_4px_10px_rgba(15,23,42,.12)]" : "text-slate-500 hover:bg-slate-100 hover:text-slate-950 active:scale-[.98]"}`}>
        <Icon size={compact ? 18 : 19} strokeWidth={selected ? 2.25 : 1.9} className="shrink-0" />
