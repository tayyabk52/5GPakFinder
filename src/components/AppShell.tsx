...
function LegalLinks({ collapsed = false, onNavigate }: { collapsed?: boolean; onNavigate?: () => void }) {
  if (collapsed) return <div className="mt-3 flex flex-col items-center gap-1 border-t border-slate-100 pt-3"><Link href="/privacy" onClick={onNavigate} title="Privacy Policy" className="grid h-9 w-9 place-items-center rounded-xl text-slate-500 transition hover:bg-slate-100 hover:text-slate-950"><ShieldCheck size={17}/></Link><Link href="/terms" onClick={onNavigate} title="Terms of Use" className="grid h-9 w-9 place-items-center rounded-xl text-slate-500 transition hover:bg-slate-100 hover:text-slate-950"><ScrollText size={17}/></Link></div>;
